...
var (
	...
	// helpStyle renders the keybind hints along the bottom of every screen.
...
// Border() so the title can sit inside the top rule.
func renderPanel(title, content string) string {
	...
	label := " " + title + " "
	fill := width - 1 - lipgloss.Width(label)
	if fill < 0 {
		fill = 0
	}
	top := borderStyle.Render(b.TopLeft+b.Top) +
		accentStyle.Render(label) +
		borderStyle.Render(strings.Repeat(b.Top, fill)+b.TopRight)
